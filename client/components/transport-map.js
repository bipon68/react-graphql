...
class TransportMap extends Component {
  ...
  render() {
    let renderStations = this.props.data.stops ? this.renderStations() : null;
    return (
      <Map
        center={center}
        zoom={17}
...
const query = gql`
  query GetStopsInArea($minLat: Float, $maxLat: Float, $minLon: Float, $maxLon: Float, $skip: Boolean = false){
    stops: stopsByBbox(minLat:$minLat, minLon: $minLon, maxLat: $minLat, maxLon: $maxLon) @skip(if: $skip){
      id
      name
      lat
      lon
